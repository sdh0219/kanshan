import { caseStore, computeStats } from '../src/services/caseStore.js';
...
console.log('\n[1] 空记录用户统计');
{
  caseStore.removeUserRecords(TEST_USER);
  const records = caseStore.getUserRecords(TEST_USER);
...
console.log('\n[4] 记录隔离：不同用户互不污染');
{
  const otherRecords = caseStore.getUserRecords('__test_other_user__');
  const myRecords = caseStore.getUserRecords(TEST_USER);
...
console.log('\n[5] 清理测试数据');
{
  const removed = caseStore.removeUserRecords(TEST_USER);
  assert('清理了4条测试记录', removed === 4, `实际清理 ${removed}`);
  assert('清理后查询为空', caseStore.getUserRecords(TEST_USER).length === 0);
}
